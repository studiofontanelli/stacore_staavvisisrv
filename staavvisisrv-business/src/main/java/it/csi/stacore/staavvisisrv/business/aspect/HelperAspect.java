package it.csi.stacore.staavvisisrv.business.aspect;
...
public class HelperAspect extends CommonAspect{
	...
	@Around(value = "execution(* it.csi.stacore.staavvisisrv.business.helper.impl..*.*(..))", argNames = "joinPoint")
	...
	@Before(value="execution(* it.csi.stacore.staavvisisrv.business.helper.impl..*.*(..))")
	public void beforeAspect(JoinPoint joinPoint) throws Throwable {
		super.beforeAspect(joinPoint);
	}

	@AfterThrowing (pointcut="execution(* it.csi.stacore.staavvisisrv.business.helper.impl..*.*(..))", throwing="ex" )
